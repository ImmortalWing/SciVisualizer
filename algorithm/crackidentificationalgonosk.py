...
def crackidentification_nosk(imagepath):
    ...
    optimal_h = int(round(cross_points[1])) if len(cross_points) > 1 else 30
    print(optimal_h)
    # 使用最优h值进行去噪
    ...
    eroded_img = cv2.erode(areathreshold_img, kernel)
...
def filtering(dilated_image):
    ...
    # 设定长度阈值
    length_threshold = 0.5 * mean_perimeter
    ...
    for cnt in contours:
        ...
        # 同时满足面积和长度条件
        if area >= area_threshold and perimeter >= length_threshold:
